terraform {
  source = "github.com/renova-cloud/renova-iac.git//terraform/modules/codebuild?ref=develop"
}

include "root" {
  path   = find_in_parent_folders("root.hcl")
  expose = true
}

include "codebuild" {
  path   = find_in_parent_folders("codebuild.hcl")
  expose = true
}

dependency "kms_backend" {
  config_path = "${dirname(find_in_parent_folders("root.hcl"))}/kms/cw_key_backend"
  
  mock_outputs = {
    key_arn = "arn:aws:ap-southeast-1:123456789012:key/mock-key-id"
  }
}

dependency "backend_task_role_arn" {
  config_path = "${dirname(find_in_parent_folders("root.hcl"))}/ecs_services/backend"
  
  mock_outputs = {
    task_role_arn = "arn:aws:iam::123456789012:role/mock-backend-task-role"
  }
}

dependency "backend_ecr_name" {
  config_path = "${dirname(find_in_parent_folders("root.hcl"))}/ecr/backend"
  
  mock_outputs = {
    repository_name = "backend"
  }
}

dependency "backend_task_execution_role" {
  config_path = "${dirname(find_in_parent_folders("root.hcl"))}/ecs_services/backend"
  
  mock_outputs = {
    execution_role_arn = "arn:aws:iam::123456789012:role/mock-backend-execution-role"
  }
}

locals {
  name       = "${include.root.locals.common_resource_config.prefix}-${include.root.locals.codebuild_configs.backend.name}"
  account_id = include.codebuild.locals.account_id
  region     = include.root.locals.vars.aws_region
}

inputs = merge(
  include.root.locals.common_resource_config,
  include.root.locals.codebuild_configs.backend,
  {
    kms_key_arn           = dependency.kms_backend.outputs.key_arn
    iam_policy_statements = templatefile("${get_terragrunt_dir()}/iam_policy_statements.tpl", {
      name                = local.name,
      account_id          = local.account_id,
      kms_key_arn         = dependency.kms_backend.outputs.key_arn,
      s3_bucket_id_artifacts = dependency.s3_bucket_id_artifacts.outputs.id
      backend_task_execution_role = dependency.backend_task_execution_role.outputs.execution_role_arn
      backend_task_role_arn      = dependency.backend_task_role_arn.outputs.task_role_arn
    })

    buildspec_file        = "${get_terragrunt_dir()}/buildspec_backend.yml"

    environment_variables = {
      AWS_DEFAULT_REGION = {
        value = local.region
        type  = "PLAINTEXT"
      }
      AWS_ACCOUNT_ID = {
        value = local.account_id
        type  = "PLAINTEXT"
      }
      IMAGE_REPO_NAME = {
        value = dependency.backend_ecr_name.outputs.repository_name
        type  = "PLAINTEXT"
      }
    }
  }
)

