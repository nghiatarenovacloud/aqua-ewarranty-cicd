...
dependency "backend_alb_sg" {
  config_path = "${dirname(find_in_parent_folders("root.hcl"))}/sg/backend_alb"
  
  mock_outputs = {
    security_group_id = "sg-mock-backend-alb"
  }
}

inputs = merge(
  include.root.locals.common_resource_config,
  include.root.locals.elb_configs.backend_alb,
  {
    # Override with actual values from dependencies
    vpc_id          = dependency.vpc.outputs.id
    subnets         = dependency.vpc.outputs.app_subnet_ids
    security_groups = [dependency.backend_alb_sg.outputs.security_group_id]
